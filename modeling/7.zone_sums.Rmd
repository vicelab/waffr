...
lc.dir <- "output/cleaned_inputs/CDL_CA_WGS84_projected"
...
make_zone_sum <- function(abs_paths, index_path) {
  z.table = zonal(raster(abs_paths), raster(index_path), "sum", progress = "text")
  return(z.table)
}
...
for (year.index in unique(year(cwr.table[["date"]]))) {
  ...
  colnames(z.table)[2:length(colnames(z.table))] <- seq(1,length(colnames(z.table))-1)
