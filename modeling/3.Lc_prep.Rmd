...
# equal to CRS("+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs")
lc.crs <- CRS("+init=epsg:4326")
kc.lut <- readRDS("output/tables/CDL_Kc_LUT_daily.rds")
kc.lut <- cbind(index = 1:nrow(kc.lut), kc.lut)
```
...
# TODO: DRY
lc.paths.cleaned <- list.files(path="output/cleaned_inputs/CDL_CA_WGS84_30m", pattern=".(tif)$", full.names=T, recursive=TRUE)
...
gdalwarp_wrapper("C:/OSGeo4W64/bin/gdalwarp.exe", "C:/OSGeo4W64/share/gdal",
                 "-s_srs EPSG:3310 -t_srs EPSG:4326 -tr 0.0003258984 0.0003259050 -r bilinear -overwrite -crop_to_cutline -co \"COMPRESS=LZW\" -co \"PREDICTOR=2\" -wm 2000 --config GDAL_CACHEMAX 2000", 
...
warp_lc <- function(abs_path, cut.shapefile, source, date) {
  ...
  gdalwarp_wrapper("bin/gdal/apps/gdalwarp.exe",
                 "-s_srs EPSG:4326 -t_srs EPSG:4326 -tr 0.0003258984 0.0003259050 -r near -overwrite -crop_to_cutline -co \"COMPRESS=LZW\" -co \"PREDICTOR=2\" -wm 2000 --config GDAL_CACHEMAX 2000", 
...
# 3 Create per-crop landcover raster masks
...
# 4 Create per crop-year crop coefficient layers [NOT USED]
