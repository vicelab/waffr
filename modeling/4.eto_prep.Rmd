...
warp_eto <- function(abs_path, cut.shapefile, source, date) {
  dir.create(paste0("D:/lbooth/wfar/output/cleaned_inputs/", source, "_projected/", year(date)), recursive = TRUE, showWarnings = FALSE)
  outpath <- paste0("D:/lbooth/wfar/output/cleaned_inputs/", source, "_projected/", year(date), "/", format(date, format="%Y_%m_%d"), ".tif")
  gdalwarp_wrapper("C:/OSGeo4W64/bin/gdalwarp.exe", "C:/OSGeo4W64/share/gdal",
                 "-s_srs EPSG:3310 -t_srs EPSG:4326 -tr 0.0003258984 0.0003259050 -r bilinear -overwrite -crop_to_cutline -co \"COMPRESS=LZW\" -co \"PREDICTOR=2\" -wm 2000 --config GDAL_CACHEMAX 2000", 
...
# Make only as many clusters as necessary, bound by available cores
cl <- makeCluster(5, outfile = "debug.txt")
